...
router.get('/Productos/:nombre?', (req, res) => {
  //aqui creo una variable para atrapar el nombre que se recibe
    let searchWord = req.params.nombre;
    ...
    if(searchWord !== null && searchWord !== '' && searchWord) {
        ...
        products = Product.find({producto: req.params.nombre, name: new RegExp(searchWord, 'i')})
            .sort({createdAt: -1});
        //sino cumple trae todo los productos
    } else {
        products = Product.find({producto: req.params.nombre}).sort({createdAt: -1});
    }
